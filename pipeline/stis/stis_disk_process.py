...
def load_imgs(dataDir, suffix='sx2'):
    """
    List of arrays containing data [0] and headers [1].
    
    dataDir: str path to directory containing FITS files.
    suffix: str suffix of FITS files to summarize.
    """
    # Ensure trailing slash in directory name.
    dataDir = os.path.join(os.path.expanduser(dataDir),'')
    
    if not os.path.exists(dataDir):
        print("\n!! **Directory {} does not exist ** !!\n".format(dataDir))
        raise OSError

    fl = np.sort(glob(dataDir + '*_{}.fits*'.format(suffix)))
    imgsHdrs = [[],[]]
    targs = []

    if len(fl) == 0:
        print("HELP!!! No FITS files found at {}".format(dataDir + '*_{}.fits*'.format(suffix)))
...
def summarize_obs(dataDir, suffix='sx2', dsName=None):
    """
    ...
    if dsName is None:
        dsName = os.path.split(dataDir[:-1])[-1]
    table_name = 'obs_log_{}.csv'.format(dsName)
    sum_table.write(dataDir + table_name, format='csv', overwrite=True)
    print("Wrote observation summary table to {}".format(dataDir + table_name))

    return


def load_obs_log(dataDir, logPath=None):

    if logPath is not None:
        logList = [logPath]
    else:
        logList = glob(dataDir + 'obs_log*.csv')
        if len(logList) == 0:
            logList = glob(dataDir + '../obs_log*.csv')
...
def combineOrbitImgs(imgs, orientats, sciInds, refInds):
    """
...
def derotate(imgs, orientats, cens):
...
if __name__ == "__main__":
    ...
    targ = args.targ
    try:
        bgCen = np.array(args.bgCen).astype(int)
    except:
        bgCen = args.bgCen #np.array(args.bgCen.split(' '), dtype=int)
    ann = args.ann
    saveFinal = args.saveFinal
# FIX ME!!! Convert suff to input arg.
    suff = 'sx2'
    psfSubMode = args.psfSubMode
    ...
    obsTable = load_obs_log(dataDir, info.get('obsLogPath'))
    ...
    print("Loading data...")
    imgsHdrs, targs, fl = load_imgs(dataDir, suffix=suff)
    ...
    if (not noCombine) and (psfSubMode.lower() not in ['pyklip-rdi']):
        orbitImgs, orientats, sciInds, refInds = combineOrbitImgs(sciImgs, orientats,
                                                              sciInds, refInds)
    else:
...
# SKIP -- for now.
    ...
    if psfSubMode.lower() in ["adi", "rdi"]:
        ...
        rotImgs = derotate(psfSubImgs, orientats[sciInds], stars[sciInds])
        ...
        spreadPAs[1:] -= np.arange(1, len(sciInds))*(90/(len(sciInds)-1))
        rotImgsBkwd = derotate(psfSubImgs, spreadPAs, stars[sciInds])
        ...
        if saveFinal:
            ...
            hdu.header['CENTRADN'] = (not args.noRadon, 'Radon transformed to get center?')
            ...
            hdu.header['FIXPIX'] = (not args.noFixPix, 'Bad pixels were fixed?')
